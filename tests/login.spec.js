...
test.describe('Login Page Automated Tests', () => {
    ...
    test('TC-01: Page should render correctly with correct title', async ({ page }) => {
        await expect(page).toHaveTitle(/Login Page/);
        await expect(page.locator('h1')).toHaveText('เข้าสู่ระบบ');
    });

    test('TC-02: Login button should be disabled by default', async ({ page }) => {
        const loginBtn = page.locator('#loginBtn');
        await expect(loginBtn).toBeDisabled();
    });

    test('TC-03: Password validation - show error for short password', async ({ page }) => {
    ...
    test('TC-04: Password validation - show error for no numbers', async ({ page }) => {
    ...
    test('TC-05: Successful validation enables login button', async ({ page }) => {
